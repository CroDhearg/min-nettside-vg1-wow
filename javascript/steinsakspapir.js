...
function spill(valg) {
    ...
    if (liv <= 0) {
        tekstboks.innerHTML = "Du har ingen liv igjen! Trykk på restart for å spille igjen.";
        return;
    }
    ...
    tekstboks.innerHTML = `Motstander valgte ${motstander}. ${resultat} Liv igjen: ${liv}`;

    // Sjekker om brukeren har gått tom for liv
    if (liv === 0) {
        tekstboks.innerHTML += " Du har ingen liv igjen!";
    }
}
...
function restartSpill() {
    ...
    tekstboks.innerHTML = "Velg stein, saks eller papir for å starte spillet!"; // Resetter meldingen
}
